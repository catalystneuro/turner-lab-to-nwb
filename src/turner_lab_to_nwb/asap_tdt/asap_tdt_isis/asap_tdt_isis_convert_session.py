...
from turner_lab_to_nwb.asap_tdt.asap_tdt_isis import AsapTdtIsisNWBConverter
...
def session_to_nwb(
    ...
    if vl_flt_file_path:
        vl_channel_metadata = session_metadata[session_metadata["Target"] == "VL"].to_dict(orient="list")
        source_data.update(
            dict(
                ProcessedRecordingVL=dict(
                    file_path=str(vl_flt_file_path),
                    channel_metadata=vl_channel_metadata,
                    es_key="ElectricalSeriesProcessedVL",
                )
            )
        )
    ...
    if gpi_flt_file_path:
        gpi_channel_metadata = session_metadata[session_metadata["Target"] == "GPi"].to_dict(orient="list")
        source_data.update(
            dict(
                ProcessedRecordingGPi=dict(
                    file_path=str(gpi_flt_file_path),
                    channel_metadata=gpi_channel_metadata,
                    es_key="ElectricalSeriesProcessedGPi",
                )
            )
        )
    ...
    converter = AsapTdtIsisNWBConverter(source_data=source_data, verbose=False)
    ...
    metadata["NWBFile"].update(
        session_id=str(channel_metadata["Filename"][0]).replace("_", "-"),
